create table APP.PAR_PARTICIPANTS (PRJ_ID bigint not null, PER_ID bigint not null) in TSBJR4; 

create table APP.PER_PERSON (PER_ID bigint generated by default as identity (start with 1), PER_DT_BIRTH varbinary(255), PER_NM_CELLPHONE varchar(255), PER_ST_FIRST_NAME varchar(255), PER_ST_LAST_NAME varchar(255), constraint PK_PER primary key (PER_ID)) in TSBJR4; 

create table APP.PRJ_PROJECT (PRJ_ID bigint generated by default as identity (start with 1), PER_DT_BEGIN varbinary(255), PER_DT_END varbinary(255), PER_ST_NAME varchar(255), PER_ST_STATUS integer, constraint PK_PRJ primary key (PRJ_ID)) in TSBJR4; 
